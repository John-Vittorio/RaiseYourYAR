// controllers/orcid.controller.js
...
export const handleOrcidCallback = async (req, res) => {
  ...
  try {
    ...
    if (!tokenData.orcid) {
      return res.status(400).json({ message: "Invalid token exchange", details: tokenData });
    }
    ...
    res.status(500).json({ message: "ORCID callback failed", error: error.message });
  }
};
